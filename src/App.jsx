  ...
  const filteredFranchises = useMemo(() => {
    ...
    return franchises.filter(
      (item) =>
        ...
        item.centre_name?.toLowerCase().includes(needle),
    )
  }, [franchises, search])
  ...
  return (
    <section className="mx-auto mt-16 max-w-7xl px-4">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <label className="fr-search inline-flex min-w-[270px] flex-1 items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm">
          <span className="text-cyan-200">Search</span>
          <input
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Search by owner name or centre..."
        ...
        <button
          ...
          className="rounded-xl border border-cyan-300/55 bg-cyan-300/15 px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-cyan-100 transition hover:bg-cyan-300/25"
        >
          Refresh Centres
        </button>
      </div>
      ...
      {!loading && filteredFranchises.length === 0 ? (
        <p className="mt-4 rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-xs text-white/75">
          No centres matched your search. Try another name or keyword.
        </p>
      ) : null}
    </section>
  )
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#0a0f1e] text-white">
      ...
      <div className="fixed left-0 top-0 z-[60] h-1 w-full bg-transparent">
        <div
          className="h-full bg-gradient-to-r from-cyan-300 via-cyan-400 to-violet-400 transition-[width] duration-150"
