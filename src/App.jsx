...
const GALLERY_IMAGES = [
  {
    ...
    title: 'Advanced Hardware Lab',
    subtitle: 'Hands-on CPU and component-level learning',
  },
  {
    url: 'https://images.unsplash.com/photo-1517430816045-df4b7de11d1d?auto=format&fit=crop&w=1200&q=80',
    title: 'Future Classroom',
    subtitle: 'Interactive projection-ready smart classes',
  },
  {
    url: 'https://images.unsplash.com/photo-1461749280684-dccba630e2f6?auto=format&fit=crop&w=1200&q=80',
    title: 'Code & Build Sessions',
    subtitle: 'Programming track with mentor-led labs',
  },
  {
    url: 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=1200&q=80',
    title: 'Network Systems Zone',
    subtitle: 'Router, switching, and cyber infrastructure',
  },
  {
    url: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=1200&q=80',
    title: 'Typing Excellence Cell',
    subtitle: 'Speed + precision drills with assessment',
  },
  {
    ...
    subtitle: 'Verification and trust systems for records',
  },
]
    ...
    <section id="about" className="mx-auto mt-16 max-w-7xl px-4">
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <aside className="glass-shell flex flex-col rounded-[1.75rem] border border-white/10 p-6">
          <div className="profile-orb mx-auto mb-4 h-40 w-40 rounded-full border border-cyan-300/50" />
...
function TechNewsGrid() {
  const updates = [
    {
      title: 'Admissions Window 2026',
      text: 'Applications for DCA, ADCA, Typing, Tally, and DCTT are now active with streamlined onboarding at all registered centres.',
      icon: Rocket,
      accent: 'cyan',
    },
    {
      title: 'Smart Lab Expansion',
      text: 'ICA has expanded practical learning capacity with upgraded workstations, network benches, and hardware diagnostics modules.',
      icon: Server,
      accent: 'violet',
    },
    {
      title: 'Placement Readiness Drive',
      text: 'Interview preparedness, communication training, and technical practicals are being integrated into long-term diploma pathways.',
      icon: MonitorSmartphone,
      accent: 'cyan',
    },
    {
      title: 'Franchise Innovation Program',
      text: 'New franchise partners receive launch kit support, curriculum standards, branding, and quality process mentoring.',
      icon: Globe,
      accent: 'violet',
    },
  ]
    ...
    <section className="mx-auto mt-14 max-w-7xl px-4">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-extrabold md:text-4xl">Technology News Wall & Student Opportunities</h2>
        </div>
        <p className="max-w-2xl text-sm text-white/75">
          Every update block below is designed in a glass-neomorphic panel with glow accents to create a premium institutional
          identity while keeping important student/franchise communication easily visible.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {updates.map((item) => (
          <article
            key={item.title}
            className={`glass-shell news-card rounded-2xl border p-5 ${
              item.accent === 'cyan' ? 'border-cyan-300/30' : 'border-violet-300/30'
            }`}
          >
            <h3 className="inline-flex items-center gap-2 text-lg font-bold">
              <item.icon size={18} className={item.accent === 'cyan' ? 'text-cyan-200' : 'text-violet-200'} />
              {item.title}
            </h3>
            <p className="mt-3 text-sm leading-7 text-white/84">{item.text}</p>
          </article>
        ))}
      </div>
    </section>
...
function CyberGallery() {
  return (
    <section id="gallery" className="mx-auto mt-16 max-w-7xl px-4">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full border border-violet-300/45 bg-violet-300/10 px-3 py-1 text-xs uppercase tracking-[0.2em] text-violet-100">
            <GalleryHorizontal size={14} />
            Visual Experience
          </p>
          <h2 className="mt-3 text-2xl font-extrabold md:text-4xl">Cyber Gallery · Labs, Classes, and Certification Events</h2>
        </div>
        <p className="max-w-xl text-sm text-white/75">
          A high-impact gallery with depth overlays, neon edge highlights, and responsive scaling to preserve performance on
          mobile while maintaining the premium “Future-Tech” aesthetic.
        </p>
      </div>

      <div className="gallery-grid">
        {GALLERY_IMAGES.map((image, index) => (
          <article key={image.title} className={`gallery-tile ${index === 0 || index === 3 ? 'gallery-xl' : ''}`}>
            <img src={image.url} alt={image.title} className="h-full w-full object-cover" loading="lazy" />
            <div className="gallery-overlay">
              <h3 className="text-lg font-bold">{image.title}</h3>
              <p className="mt-1 text-xs text-white/85">{image.subtitle}</p>
            </div>
          </article>
        ))}
      </div>
...
function FranchiseCommandCenter() {
  return (
    <section id="franchise" className="mx-auto mt-16 max-w-7xl px-4">
      <div className="franchise-panel glass-shell rounded-[1.75rem] border border-white/15 p-6 md:p-8">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <article>
            ...
            <h2 className="mt-4 text-2xl font-extrabold md:text-4xl">Launch a High-Trust Computer Academy Franchise with ICA</h2>
            <p className="mt-4 text-sm leading-7 text-white/84 md:text-base">
              ICA supports ambitious educators and entrepreneurs to establish modern computer education centres with a structured
              curriculum, administrative frameworks, marketing support, and verification-backed credibility systems.
            </p>
            <ul className="mt-5 grid gap-2 text-sm text-white/85">
              <li className="inline-flex items-center gap-2"><CheckCircle2 size={15} className="text-emerald-300" /> Branding, admission workflow, and launch support.</li>
              <li className="inline-flex items-center gap-2"><CheckCircle2 size={15} className="text-emerald-300" /> Academic operations playbook and course delivery guide.</li>
              <li className="inline-flex items-center gap-2"><CheckCircle2 size={15} className="text-emerald-300" /> Verification-driven trust architecture for students.</li>
              <li className="inline-flex items-center gap-2"><CheckCircle2 size={15} className="text-emerald-300" /> Ongoing quality mentorship from ICA core team.</li>
            </ul>
          </article>

          <aside className="rounded-2xl border border-white/20 bg-white/5 p-4">
            <h3 className="mb-3 text-lg font-bold text-cyan-100">Franchise Enquiry Pipeline</h3>
            <label className="cyber-input mb-3">
              <span className="mb-1 block text-xs uppercase tracking-[0.14em] text-cyan-100">Applicant Name</span>
              <input className="w-full bg-transparent text-sm text-white placeholder:text-white/45 focus:outline-none" placeholder="Your full name" />
            </label>
            <label className="cyber-input mb-3">
              <span className="mb-1 block text-xs uppercase tracking-[0.14em] text-cyan-100">City / State</span>
              <input className="w-full bg-transparent text-sm text-white placeholder:text-white/45 focus:outline-none" placeholder="Where do you want to open?" />
            </label>
            <label className="cyber-input mb-4">
              <span className="mb-1 block text-xs uppercase tracking-[0.14em] text-cyan-100">Contact Number</span>
              <input className="w-full bg-transparent text-sm text-white placeholder:text-white/45 focus:outline-none" placeholder="+91..." />
            </label>
            <button type="button" className="w-full rounded-xl border border-cyan-300/55 bg-cyan-300/15 px-4 py-2 text-sm font-semibold text-cyan-100 transition hover:bg-cyan-300/25">
              Request Franchise Callback
            </button>
          </aside>
        </div>
      </div>
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#0a0f1e] text-white">
      <style>{`
        ...
        .gallery-overlay {
          position: absolute;
          inset: auto 0 0 0;
          background: linear-gradient(180deg, rgba(8, 13, 28, 0), rgba(8, 13, 28, 0.8) 45%, rgba(8, 13, 28, 0.95));
        ...
        .franchise-panel::before {
          ...
          position: absolute;
          inset: -35%;
          background: conic-gradient(from 120deg, rgba(139, 92, 246, 0.2), rgba(34, 211, 238, 0.2), transparent, transparent);
          animation: rotate-haze 16s linear infinite;
          pointer-events: none;
        }
      ...
      <main className="pt-[110px] md:pt-[120px]">
        <section className="relative isolate overflow-hidden px-4 pb-16 pt-16 md:pb-20 md:pt-20">
          ...
          <div className="reveal-item relative mx-auto max-w-7xl">
